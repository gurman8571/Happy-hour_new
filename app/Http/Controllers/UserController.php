...
class UserController extends Controller
{
    ...
    public function Profile(Type $var = null)
    {
        ...
        $user=Places::with('categories')->where('id',session()->get('user-login-id'))->first();
        //dd($user);
            ...
            public function edit(request $req)
            {
                ...
                $validatedData=$req->validate([
                    ...
                    'location'=>'required',
                     'website'=>'required', 
                    'marketing_name'=>'required',
                    'marketing_phone' => 'required|numeric|digits:12'
                    //'mobile'=> ''
                   ]);
    ...
    if($storage){
		...
		return back()->with('error', 'something went wrong!');
            ...
            public function Offers(Type $var = null)
            {
               ...
               $offers=Offer::where('place_id',$placeId)->orderBy('id','DESC')->get();
               //dd($offers);
            ...
            public function OfferPost(request $req)
            {
               ...
               return redirect()->route('UserOffers')->with('success','offer added');
            }
            ...
            public function CreateOffer()
            {
                ...
                public function notify(request $req ,$id)
                {
                     ...
                     $header=$offer->Heading;
                       ...
                       //dd($placeId);
                       $users=Favourite::where('placeId',$placeId)->pluck('userId');
                       $image=Places::where('id',$placeId)->first();
                       ...
                       //$notification->userId=$users[$i];
                       $notification->placeId=$placeId;
                       $notification->message=$message;
                       $notification->heading=$header;
                       ...
                       return redirect()->route('UserOffers')->with('success','Notification send');
                ...
                {
                    $Placecategories=Place_category::where('places_id',session()->get('user-login-id'))->pluck('category_id')->toarray();
                    $categories=Category::pluck('categoryName','id');
                ...
                public function UserAvailibilityEditTime(request $req)
                {
                   ...
                   $data->end=$req->end;
                   $data->save();
                   return redirect()->route('user.availability')->with('success','Time updated successfullty');
                }
...
public function resetPassword($token){
    ...
    if (!$user) return redirect()->back()->withError('Email not found');
